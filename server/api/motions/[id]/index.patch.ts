...
import { motionDraftUpdateSchema } from '../../../utils/validation'
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readValidatedBody(event, motionDraftUpdateSchema.parse)
  ...
  if (body.topic !== undefined) {
    updates.topic = resolveDraftTopic(body.topic, existing.topic)
  }
  ...
  if (body.bodyHtml !== undefined) {
    updates.bodyHtml = normalizeDraftBodyHtml(body.bodyHtml)
  }
